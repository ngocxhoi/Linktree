...
const { allLinks, updatedLinkId } = storeToRefs($userStore);
...
const getLinkById = () => {
  currentLink.value = allLinks.value.find(
    (link) => link.id == updatedLinkId.value
  );
  name.value = currentLink.value.name;
  url.value = currentLink.value.url;
};
...
const updateLinkImage = async () => {
  try {
    let res = await useApiFetch(
      `/api/link-image/${currentLink.value.id}`,
      "POST",
      data.value
    );
    if (res?.error.value) throw res.error.value.data;
    else {
      currentLink.value.image = res?.data.value.image;
    }
  } catch (error) {
    console.log(error);
  } finally {
...
const deleteLink = async () => {
  ...
  if (res) {
    try {
      ...
      let res = await useApiFetch(
        `/api/links/${currentLink.value.id}`,
        "DELETE"
      );
      if (res?.error.value) throw res.error.value.data;
      else {
        allLinks.value.splice(allLinks.value.indexOf(currentLink.value), 1);
      }
    } catch (error) {
      console.log(error);
      errors.value = error;
...
const updateLink = useDebounce(async () => {
  ...
  try {
    ...
    if (res?.error.value) throw res.error.value.data;
      ...
      currentLink.value = res?.data.value.link;
    }
  } catch (error) {
    console.log(error);
    errors.value = error;
